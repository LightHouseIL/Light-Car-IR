...
//% color=#008B00 icon="\uf1eb"
//% weight=10
// The following globals are provided by the MakeCode micro:bit environment:
// DigitalPin, pins, PinPullMode, PulseValue, input, basic, control, randint
// linter-ignore
namespace makerbit {
  let irState: IrState;
    ...
    if (irEvent === IR_DATAGRAM) {
      ...
      if (newCommand !== irState.activeCommand) {
        ...
        if (irState.activeCommand >= 0) {
          const releasedHandler = irState.onIrButtonReleased.find((h: IrButtonHandler) => h.irButton === irState.activeCommand || IrButton.Any === h.irButton);
          if (releasedHandler) {
            background.schedule(releasedHandler.onEvent, background.Thread.UserCallback, background.Mode.Once, 0);
          }
        }

        const pressedHandler = irState.onIrButtonPressed.find((h: IrButtonHandler) => h.irButton === newCommand || IrButton.Any === h.irButton);
        if (pressedHandler) {
  ...
  function notifyIrEvents() {
    if (irState.activeCommand === -1) {
      ...
      if (now > irState.repeatTimeout) {
        ...
        const handler = irState.onIrButtonReleased.find((h: IrButtonHandler) => h.irButton === irState.activeCommand || IrButton.Any === h.irButton);
        if (handler) {
